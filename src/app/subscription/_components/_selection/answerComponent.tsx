...
export function AnswerComponent({
  ...
  return (
    <label className='h-full cursor-pointer' htmlFor={`${passKey}-${value}`}>
      <input
        type='radio'
        id={`${passKey}-${value}`}
        name={passKey}
        ...
        aria-describedby={`${passKey}-${value}-desc`}
      />
      ...
      <div className='hover:bg-accent-secondary peer-checked:bg-brand-primary! text-body peer-checked:text-body-inverted h-full rounded-[8px] bg-[#F4F1EB] p-6 peer-focus-visible:outline-2 md:px-6 md:py-8 lg:px-7 lg:py-8'>
        ...
        <p className='content-text mt-2 text-left md:mt-6' id={`${passKey}-${value}-desc`}>
          {displayPrice}
          {description}
        </p>
      </div>
